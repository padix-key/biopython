...
def apply_residue_wise(array, data, function, axis=None):
    ...
    The function takes an atom array (stack) and an data array (`ndarray`) of
    the same length. The function iterates through the residue IDs of the atom
    array (stack) and identifies intervals of the same ID. Then the data is
    partitioned into the same intervals, and each interval (also an
    `ndarray`) is put as parameter into `function`. Each return value is stored
    as element in the resulting `ndarray`, therefore each element corresponds
    to one residue. 
    ...
    data : ndarray
        The data, whose intervals are the parameter for `function`. Must have
        same length as `array.res_id`.
    function : function
        The `function` must have either the form *f(data)* or *f(data, axis)*
        in case `axis` is given. Every `function` call must return a value
    ...
    processed_data : ndarray
        ...
        first dimension of this array is equal to the amount of residues.
...
def get_residues(array):
    ...
    The residues are listed in the same order they occur in the array (stack).
